...
\begin{document}
\begin{frontmatter}
\author{Matteo Francia\corref{cor1}}\ead{m.francia@unibo.it}
\cortext[cor1]{Corresponding author.}
\author{Enrico Gallinucci}\ead{enrico.gallinucci@unibo.it}
\author{Matteo Golfarelli}\ead{matteo.golfarelli@unibo.it}
% \affiliation{organization={DISI -- University of Bologna},
...
In \Cref{sec:Overview}, we briefly describe CTM and motivate its adoption.
...
\Cref{tab:related} summarizes the comparison; the strengths of CTM are the following.
\begin{enumerate}
...
\begin{itemize}
    \item The tessellation granularity defines the level of the analysis; for instance, CTM transparently allows the extraction of patterns at neighborhood/city/country scales (\Cref{fig:multilevtess}).
...
With reference to \Cref{fig:patterns2}, where a regular tessellation is used for simplicity, $T_b,T_g$, and $T_r$ are trajectories while $Q_{A1}$ and $Q_{B2}$ are transactions that correspond to tiles A1 and B2.
{\small
\begin{align*}
    T_b&= (A1, B1, C1, D1, C1),
...
Note that approaching this problem as a colossal itemset mining rather than a typical clustering one (see \Cref{sec:related}):
\begin{itemize}
...
This ``basic'' co-movement pattern can be specialized into those reported in \Cref{tab:proximitypattern} depending on the involved features, either space ($f^{sp}$) or space and time ($f^{tm}$), and on the additional shape constraints described in \Cref{tbl:constraints2}.
...
As summarized in \Cref{tbl:constraints2}, co-location and flow are defined over a spatial feature ($f^{sp}$; i.e., they are required to happen in the same spatial tile), while swarm and convoy require both spatial ($f^{sp}$) and temporal ($f^{tm}$) features (i.e., they are required to happen in the same spatio-temporal tiles).
...
% }{
% In CTM, behavioral constraints are gracefully and transparently applied through the tessellation just by adding the corresponding features to those in the feature space.
% According to \Cref{def:tes}, two or more trajectories move through the tile $s$ if they share the same feature values for all the features in the feature space $F$. In other words, the support of an itemset $I$ (i.e., a set of trajectories), includes all and only the transactions (i.e., the tiles) where the items in $I$ share the same feature values. 
% }
...
\Cref{tab:effpatterns} reports the outcomes for all datasets and all co-movement pattern types.
\begin{figure}[t]
    \centering
